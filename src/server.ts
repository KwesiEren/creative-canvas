...
const knowledgeChunks: KnowledgeChunk[] = [
  {
    ...
    embedding: buildMockEmbedding(KNOWLEDGE.summary),
  },
  {
  ...
  },
  {
    ...
    embedding: buildMockEmbedding(
      KNOWLEDGE.programmes.map((programme) => `${programme.name} ${programme.id}`).join(" "),
    ),
  },
  {
    id: "notes",
  ...
  },
  {
    ...
    embedding: buildMockEmbedding(
      `Office ${KNOWLEDGE.contact.office} email ${KNOWLEDGE.contact.email}`,
    ),
  },
];

const CHUNK_PATH: Record<string, string> = {
  summary: "home",
  "information-architecture": "about",
  "migration-rules": "resources",
  programmes: "programmes",
  notes: "knowledge-hub",
  contact: "contact",
};

function retrieveKnowledgeAnswer(query: string): {
  text: string;
  sources: { label: string; path: string }[];
} {
  const normalizedQuery = query.trim();
  if (!normalizedQuery) {
    return { text: KNOWLEDGE.summary, sources: [{ label: "ADF summary", path: "home" }] };
  }
    ...
    try {
      ...
      try {
        ...
        if (url.pathname === "/api/chat" && request.method === "POST") {
          ...
          const { text, sources } = retrieveKnowledgeAnswer(message);
